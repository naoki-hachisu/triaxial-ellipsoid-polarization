...
* **`03_generate_polarization_database.py`**
  Generates a comprehensive database containing the degree of polarization ($P_i$), polarization angle ($\chi$), and Stokes parameters ($I, Q, U$) over a 4-dimensional parameter space: shape parameters ($\log a, \log b$) and viewing angles ($\theta, \phi$). It also provides a rough estimation of the maximum degree of polarization ($P_{i,\rm max}$) and its corresponding viewing direction ($\theta_{\rm max}, \phi_{\rm max}$) and angle ($\chi_{\rm max}$). 
